...
import type { ToolMetadata, ToolDiscoveryOptions } from '../interfaces/tool-registry.interface';
    ...
    try {
      ...
      watcher.on('unlink', (filePath: string) => {
        if (/\.tool\.[jt]s$/.test(filePath)) {
          ...
          const remainingTools = Array.from(this.discoveredTools.values()).filter(
            tool => tool.location.file !== filePath
          );
