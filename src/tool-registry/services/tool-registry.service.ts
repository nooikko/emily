...
export class ToolRegistryService implements ToolRegistry, OnModuleInit {
  ...
  search(query: ToolSearchQuery): ToolRegistration[] {
    ...
    for (const [_, registration] of this.tools) {
      ...
      if (query.permissions && query.permissions.length > 0) {
        const hasRequiredPermissions = query.permissions.every((permission) => metadata.permissions?.some((p) => p.role === permission));
   ...
   */
  private createLangChainTool(metadata: ToolMetadata, schema?: z.ZodSchema<any>, handler?: Function): StructuredToolInterface {
      ...
      func: async (input) => {
        ...
        try {
          ...
          if (metadata.deprecated) {
            this.logger.warn(`Tool ${metadata.name} is deprecated: ${metadata.deprecationMessage || 'No message provided'}`);
          }
          ...
          if (metrics) {
            ...
            metrics.averageExecutionTime = (metrics.averageExecutionTime * (metrics.executions - 1) + executionTime) / metrics.executions;
  ...
  private async discoverFromFileSystem(options: ToolDiscoveryOptions): Promise<void> {
    ...
    for (const searchPath of paths) {
      const files = await glob(recursive ? `${searchPath}/**/*` : `${searchPath}/*`, { nodir: true });
  ...
  private findLatestToolKey(name: string): string | null {
    ...
    const latest = versions.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())[0];
