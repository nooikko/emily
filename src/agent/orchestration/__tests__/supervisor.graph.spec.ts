  ...
  beforeEach(async () => {
    ...
    jest.spyOn(compiledGraph, 'invoke').mockImplementation(async (inputState: unknown, _config?: unknown) => {
      ...
      const result: SupervisorState = {
        ...inputState,
        currentPhase:
          inputState.currentPhase === 'planning'
            ? 'execution'
            : inputState.currentPhase === 'execution'
              ? 'consensus'
              : inputState.currentPhase === 'consensus'
                ? 'review'
                : 'complete',
        messages: [
          ...inputState.messages,
        ...
        routingDecision:
          allTasksCompleted || (inputState.currentPhase === 'execution' && inputState.agentTasks?.every((t: AgentTask) => t.status === 'completed'))
            ? 'No pending tasks'
