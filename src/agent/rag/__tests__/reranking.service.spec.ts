...
import { RerankingService, RerankingRetriever } from '../services/reranking.service';
...
describe('RerankingService', () => {
  ...
  describe('applyMMRReranking', () => {
    ...
    it('should prioritize relevance when lambda is high', async () => {
      ...
      const results = await service.applyMMRReranking(
        sampleDocuments, 
        query, 
        { lambda: highRelevanceLambda }
      );
  ...
  describe('applyHybridReranking', () => {
    ...
    it('should handle different fusion methods', async () => {
      ...
      const weightedResults = await service.applyHybridReranking(
        sampleDocuments, 
        query, 
        mockLLM,
      ...
      // Test RRF fusion
      const rrfResults = await service.applyHybridReranking(
        sampleDocuments, 
        query, 
        mockLLM,
        { fusionMethod: 'rrf' }
      ...
      const bordaResults = await service.applyHybridReranking(
        sampleDocuments, 
        query, 
        mockLLM,
  ...
  describe('analyzeRerankingEffectiveness', () => {
    ...
    it('should analyze reranking effectiveness', async () => {
      ...
      const analysis = await service.analyzeRerankingEffectiveness(
        originalDocuments,
        mockRerankedResults,
        query
      );
    ...
    describe('extractScoreFromLLMResponse', () => {
      it('should extract score from various response formats', () => {
        const responses = [
          'Score: 8.5',
          'Relevance: 7/10',
          'The relevance score is 9.2 out of 10',
          'Rating: 6/10',
          'No clear score here'
        ];
  ...
  describe('error handling and validation', () => {
    ...
    it('should handle errors in hybrid reranking gracefully', async () => {
      ...
      await expect(
        service.applyHybridReranking([sampleDocuments[0]], 'test query', errorLLM)
