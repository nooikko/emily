...
import { EnsembleRetrieverService, EnsembleRetriever } from '../services/ensemble-retriever.service';
...
describe('EnsembleRetrieverService', () => {
  ...
  describe('executeEnsembleRetrieval', () => {
    ...
    it('should include metadata when requested', async () => {
      ...
      const results = await service.executeEnsembleRetrieval(
        ensembleRetriever, 
        query, 
    ...
    it('should handle parallel vs sequential execution', async () => {
      ...
      const parallelResults = await service.executeEnsembleRetrieval(
        ensembleRetriever, 
        query, 
        { parallelize: true }
      );
      ...
      const sequentialResults = await service.executeEnsembleRetrieval(
        ensembleRetriever, 
        query, 
        { parallelize: false }
      );
  ...
  describe('createMultiModalRetriever', () => {
    const mockImageRetriever = createMockRetriever('image', [
      new Document({ pageContent: 'Image description 1', metadata: { type: 'image' } })
    ]);

    const mockAudioRetriever = createMockRetriever('audio', [
      new Document({ pageContent: 'Audio transcription 1', metadata: { type: 'audio' } })
    ]);
  ...
  describe('error handling and validation', () => {
    ...
    it('should warn about weight normalization', () => {
      ...
      expect(loggerSpy).toHaveBeenCalledWith(
        expect.stringContaining("Weights don't sum to 1"),
        expect.any(Object)
      );
