...
describe('Context-Aware Personality Switching Integration', () => {
  ...
  let switchingOrchestrator: PersonalitySwitchingOrchestratorService;
  ...
  let personalityProfileRepository: Repository<PersonalityProfile>;
  let conversationThreadRepository: Repository<ConversationThread>;
  ...
  const mockCasualPersonality: PersonalityProfile = {
    ...
    meetsConditions: function() { return true; },
  ...
  beforeEach(async () => {
    ...
    contextAwareSwitchingService = module.get<ContextAwarePersonalitySwitchingService>(
      ContextAwarePersonalitySwitchingService
    );
    ...
    switchingOrchestrator = module.get<PersonalitySwitchingOrchestratorService>(PersonalitySwitchingOrchestratorService);
    ...
    personalityProfileRepository = module.get<Repository<PersonalityProfile>>(
      getRepositoryToken(PersonalityProfile)
    );
    conversationThreadRepository = module.get<Repository<ConversationThread>>(
    ...
    jest.spyOn(personalityService, 'findOne').mockImplementation((id: string) => {
      ...
      } else if (id === 'technical-expert-1') {
        return Promise.resolve(mockTechnicalPersonality);
      }
      return Promise.reject(new Error('Personality not found'));
    });

    jest.spyOn(personalityService, 'findAll').mockResolvedValue([
      mockCasualPersonality,
      mockTechnicalPersonality
    ]);
  });
  ...
  describe('Context Analysis and Switching Decision', () => {
    it('should analyze conversation context correctly for casual conversation', async () => {
      ...
      // Act
      const contextAnalysis = await contextAnalyzer.analyzeConversationContext(
        messages,
        undefined,
        'casual-assistant-1'
      );
    ...
    it('should detect technical context and recommend switching', async () => {
      ...
      const messages = [
        ...
        new HumanMessage({ content: 'I need to implement a complex algorithm for optimizing database queries with async/await patterns and proper error handling.' }),
      ];
    ...
    it('should score personality compatibility accurately', async () => {
      ...
      const casualScore = await compatibilityScorer.scorePersonalityCompatibility(
        'casual-assistant-1',
        technicalContext
      );

      // Score technical personality (should be high)
      const technicalScore = await compatibilityScorer.scorePersonalityCompatibility(
        'technical-expert-1',
        technicalContext
      );
  ...
  describe('Transition Smoothing', () => {
    it('should create smooth transitions between personalities', async () => {
      // Arrange
      const messages = [
        new HumanMessage({ content: 'Let\'s talk about algorithms now.' }),
      ];
    ...
    it('should optimize transition configuration based on personality distance', async () => {
      // Act
      const optimizedConfig = await transitionSmoother.optimizeTransitionConfig(
        'casual-assistant-1',
        'technical-expert-1',
        {
          messageCount: 10,
  ...
  describe('State Tracking and Evolution', () => {
    it('should track personality state changes over conversation', async () => {
      ...
      const snapshot1 = await stateTracker.createStateSnapshot(
        threadId,
        'casual-assistant-1',
        messages.slice(0, 2),
        undefined,
      ...
      const snapshot2 = await stateTracker.createStateSnapshot(
        threadId,
        'technical-expert-1',
        messages,
        undefined,
  ...
  describe('Conversation Monitoring', () => {
    it('should identify switching opportunities during ongoing conversation', async () => {
      ...
      // Act
      const monitoringResult = await contextAwareSwitchingService.monitorConversationForSwitching(
        threadId,
        messages,
        'casual-assistant-1'
      );
  ...
  describe('System Analytics and Optimization', () => {
    ...
    it('should handle configuration management correctly', async () => {
      ...
      const messages = [
        new HumanMessage({ content: 'I need expert-level programming help.' }),
      ];
      const switchingResult = await contextAwareSwitchingService.performContextAwareSwitch(
        threadId,
        messages,
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle missing personality gracefully', async () => {
      ...
      // Act & Assert
      await expect(
        contextAwareSwitchingService.performContextAwareSwitch(
          'test-thread',
          messages,
  ...
  describe('Performance and Scalability', () => {
    it('should handle multiple concurrent switching requests', async () => {
      ...
      const messages = [
        new HumanMessage({ content: 'Help me with technical problems.' }),
      ];
    ...
    it('should maintain reasonable processing times', async () => {
      // Arrange
      const messages = [
        new HumanMessage({ content: 'I need comprehensive help with advanced machine learning algorithms, neural network optimization, and performance tuning strategies.' }),
      ];
