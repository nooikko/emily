...
import { 
  CreatePersonalityProfileDto,
  PersonalitySearchDto,
  SwitchPersonalityDto 
} from '../dto/create-personality-profile.dto';
...
import { ErrorDto, ValidationErrorDto, ConflictErrorDto, NotFoundErrorDto } from '../../common/dto/error.dto';
...
export class PersonalityProfileController {
      ...
      codingAssistant: {
        ...
        value: {
          ...
          promptTemplates: [
            {
              type: 'system',
              template: 'You are a professional coding assistant with {expertise_level} knowledge. Maintain a {tone} tone and provide {communication_style} responses.',
  ...
  ): Promise<PersonalityRecommendationResponseDto[]> {
    if (!context) {
      throw new BadRequestException('Context is required for recommendations');
    }

    const recommendations = await this.personalityProfileService.recommendPersonalities(
      context,
      limit || 5
    );
  ...
  async switchPersonality(@Body() switchDto: SwitchPersonalityDto): Promise<PersonalitySwitchResponseDto> {
    ...
    const compiledTemplate = await this.personalityProfileService.switchPersonality(
      switchDto.personalityId,
      {
    ...
    const preview = await this.templateService.formatTemplatePreview(
      compiledTemplate,
      switchDto.userPreferences || {}
    );
  ...
  async update(
    ...
    @Body() updateDto: UpdatePersonalityProfileDto
  ): Promise<PersonalityProfileResponseDto> {
  ...
  async exportAll(
    @Query('includeSystemPersonalities') includeSystem?: boolean
  ): Promise<DetailedPersonalityProfileResponseDto[]> {
